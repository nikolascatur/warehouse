...
export const GoodList: React.FC = () => {
  const fetcher = async (
    url: string
  ): Promise<WebResponse<GoodsResponse[]>> => {
    ...
    <div>
      <div className="grid grid-flow-row">
        <div className="grid grid-flow-col grid-cols-7" key="title">
          <div
            className="border-solid border-2 flex justify-center font-bold bg-slate-500"
            key="No"
          >
            No
          </div>
        ...
        {data?.data?.map((it) => (
          <div className="grid grid-flow-col grid-cols-7" key={it.id}>
            <div
              className="border-solid border-2 flex justify-center font-thin"
              key={it.id + 1}
            >
              {it.index}
            </div>
      ...
      <div className="flex flex-row w-full justify-center">
        <button key={"button_add_good"} onClick={() => setIsOpenDialog(true)}>
          Tambah Barang
        </button>
        <DialogForm
          isOpen={isOpenDialog}
